...
const CampaignForm = () => {
  ...
  const validateForm = () => {
    ...
    else if (!/^\d+$/.test(formData.phone)) newErrors.phone = 'Please enter a valid phone number (digits only).';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_URL_PREFIX}/api/apiHome/campaign/form`, {
        method: 'POST',
        headers: { 
  ...
  return (
    <Suspense fallback={<Spinner />}>
      <form onSubmit={handleSubmit} className={styles.form} noValidate>
        ...
        <p className={styles.formSubtitle}>Fill in your details and we&#39;ll help you plan your perfect trip.</p>
        ...
        <div className={styles.formGrid}>
          ...
          <div className={`${styles.formGroup} ${styles.span2}`}>
            ...
            <div className={styles.inputGroup}>
              ...
              <input
                ...
                onChange={handleInputChange}
                className={`${styles.input} ${styles.contactInput} ${errors.phone ? styles.inputError : ''}`}
                required
                placeholder="Your phone number"
                aria-describedby="phone-error"
              />
            </div>
            {errors.phone && <p id="phone-error" className={styles.errorMessage}>{errors.phone}</p>}
          </div>

          <div className={styles.formGroup}>
            <label htmlFor="travelDate" className={styles.label}>Tentative Travel Date</label>
            <select
              id="travelDate"
              name="travelDate"
          ...
          <div className={`${styles.formGroup} ${styles.span2}`}>
            <label className={styles.label}>Do you need help deciding on a destination?</label>
